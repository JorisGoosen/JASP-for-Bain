...
.bainTwoSampleTable <- function(dataset, options, jaspResults, missingValuesIndicator) {
  ...
  if(options$hypothesis == "_4type"){
    type <- 4
    message <- "The hypothesis H1 specifies that the mean of group 1 is bigger than the mean of group 2. The hypothesis H2 specifies that the mean in group 1 is smaller than the mean in group 2. The posterior probabilities are based on equal prior probabilities."
  }
  if(options$hypothesis == "allTypes"){
      type <- 5
