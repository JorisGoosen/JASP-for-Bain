...
.bainOneSampleTable <- function(dataset, options, jaspResults, missingValuesIndicator) {
  ...
  if(options$hypothesis == "_4type"){
    type <- 4
    message <- paste0("The hypothesis H1 specifies that the mean is bigger than ",options$testValue," and the hypothesis H2 specifies that the mean is smaller than ",options$testValue,". The posterior probabilities are based on equal prior probabilities.")
  }
  if(options$hypothesis == "allTypes"){
      type <- 5
